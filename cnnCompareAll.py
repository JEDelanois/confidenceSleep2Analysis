...
fullSleepData.createSimulationStructureFromPattern( \
    "/bazhlab/edelanois/cnnSleep/34/simulations/genAlgoBest/sim4212/" \
    , "Cnn Sleep" \
    ,[] \
    , range(0,1)) 
...
# allDatasets = [*set(allDatasets)]


picklePath = "./finetune.pkl"
quickloadedFinetune = False
if os.path.exists(picklePath) and not forceReload:
    ...
    for datasetname in allDatasets:
        ...
        curData.createSimulationStructureFromPattern( \
            "../simulationSweep/finetuneSweep/dataset-%s/" % datasetname.replace("task1ValidData", "task1TrainData") \
            , datasetname \
            ,[] \
            , range(0,1)) 
...
datas = [fullSleepData, fullSleepData]
timePoints = [0, 1]
timePointsPrettyNames = ["Post Training", "Sleep"]

# for i,datasetNames1 in enumerate(dataGroups):
    # groupName1 = getGroupName(datasetNames1)
    # extraDatas = [d for d in allFinetune if d.titlePattern in datasetNames1]
    ...
    # Metrics.Basics.plotMetricTable(datas + extraDatas, datsetNames=datasetNames1, timePoints=timePoints + extraTimePoints, metricName="matlabAcc", timePointsPrettyNames=timePointsPrettyNames + extraTimePointsPrettyNames, prettyXTicks=True, prettyFileName=groupName1, prettyXLabel=None)
...
for i,datasetNames1 in enumerate(dataGroups): # fine tune models
    for j,datasetNames2 in enumerate(dataGroups): # datasets tested on
        groupName1 = getGroupName(datasetNames1)
        groupName2 = getGroupName(datasetNames2)
        extraDatas = [d for d in allFinetune if d.titlePattern in datasetNames1]
        extraTimePoints = [-1] * len(extraDatas)
        extraTimePointsPrettyNames = [d.titlePattern.replace("task1TrainData", "task1ValidData") for d in extraDatas]
        Metrics.Basics.plotMetricTable(datas + extraDatas, datsetNames=datasetNames2, timePoints=timePoints + extraTimePoints, metricName="matlabAcc", timePointsPrettyNames=timePointsPrettyNames + extraTimePointsPrettyNames, prettyXTicks=True, prettyFileName="models-%s_data%s" % (groupName1, groupName2), prettyXLabel=None)
        Metrics.Basics.plotSpecificTrialMetricOverDatasetValue(
            datas + extraDatas
            , datsetNames=datasetNames2
            , datsetValues=valueGroup
            , timePoints=timePoints + extraTimePoints
            , metricName="matlabAcc"
            , timePointsPrettyNames=timePointsPrettyNames + extraTimePointsPrettyNames
            , prettyXTicks=True
            , prettyFileName="models-%s_data-%s-lineMatlabAcc.png" % (groupName1, groupName2)
            , prettyXLabel=None)
